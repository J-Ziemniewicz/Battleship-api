...
class Player(tornado.websocket.WebSocketHandler):
    ...
    def __parseMsgType(self, msg: dict):
        if msg['type'] == 'playerId':
            ...
            activePlayers.remove(self.__playerId)
            currentPlayers[int(msg['playerId'])] = currentPlayers.pop(
                self.__playerId)
            ...
            self.send_message(
                {'type': 'newGame', 'gameId': newGame.getGameId()})
        elif msg['type'] == 'exitGame':
            oponentId = self.__getOponentid(
                int(msg['gameId']), self.__playerId)
            if(oponentId != 0):
